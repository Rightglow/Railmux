...
| Native Windows synchronized provider redraw | The Railmux-owned MSYS2 generation installs and validates tmux 3.7 or newer before activation. Users do not maintain this private tmux. Windows Terminal entries use a same-session private PTY that leaves ordinary output byte-exact and coalesces only observed high-frequency cursor visibility while preserving provider animation and Working timers; user config/history is unchanged. The shared macOS/Linux/WSL core floor remains tmux 2.7. |
