...
| Native Windows synchronized provider redraw | The Railmux-owned MSYS2 generation installs and validates tmux 3.7 or newer before activation. Users do not maintain this private tmux. Windows Terminal entries use a private PTY that keeps the cursor visible across paired repaint noise and restores a proven quiet input row inside synchronized paints while preserving provider animation and Working timers. During the same burst, the newest complete repaint of that proven row may be deferred to the 100 ms quiet boundary when a following absolute cursor position makes its immediate cursor advance irrelevant. Because Windows IME composition can expose temporary ASCII/`DEL` bytes, each input read extends an independent 100 ms guard for only that proven prompt row; its newest authoritative contents publish once input quiets even while other rows remain live. Incomplete/unknown/screen-wide output stays authoritative. Same-row caret movement remains live; position debt, intentional hides, resize, and relative/unknown movement retain fail-safe authority. User input/config/history is unchanged. The shared macOS/Linux/WSL core floor remains tmux 2.7. |
