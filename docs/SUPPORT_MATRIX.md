...
| Native Windows synchronized provider redraw | The Railmux-owned MSYS2 generation installs and validates tmux 3.7 or newer before activation. Users do not maintain this private tmux. Windows Terminal entries use a private PTY that coalesces paired cursor noise and atomic prompt repaints. Because Codex may scroll the complete grid underneath terminal-owned IME pre-edit, composition-shaped text input now gates the complete provider-output stream while input itself reaches the PTY immediately; committed UTF-8 releases on a short boundary, Enter/Ctrl-C release immediately, and a bounded overflow fails open without dropping bytes. The settled batch is one synchronized-output transaction. Provider animation remains live when the user is not composing. User input/config/history is unchanged, and Ctrl-C targets a focused provider while retaining the controller's quit action. The shared macOS/Linux/WSL core floor remains tmux 2.7. |
