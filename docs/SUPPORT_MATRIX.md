...
| Native Windows synchronized provider redraw | The Railmux-owned MSYS2 generation installs and validates tmux 3.7 or newer before activation. Users do not maintain this private tmux. Windows Terminal entries consume their private tmux PTY into the shared `railmux ssh` VT screen model, defer sampling through bounded application synchronized-output frames, coalesce a busy restore burst to its latest available screen, and atomically paint only final changed rows plus the authoritative cursor. A single-flight physical writer prevents ConPTY backpressure from blocking input or the tmux watchdog and never queues stale dependent patches. Working animation and elapsed time remain live while an unchanged prompt row is not touched beneath terminal-owned IME pre-edit. Input remains immediate and byte-exact; Ctrl-C targets a focused provider while retaining the controller's quit action. Renderer failure stops only the attach client and has no raw-output fallback. The shared macOS/Linux/WSL core floor remains tmux 2.7. |
...
| P06 | Live open on click/Enter; live scrolling on wheel; canonical history on Space or Preview | Supported | Supported | POSIX/WSL direct wheel input remains tmux/provider-native. Native Windows and `railmux ssh` share bounded per-pane scrolling, preserve local wheel distance while coalescing paints, use one row per POSIX/macOS tick and a three-row step on managed Windows, and normally use styled raw pane capture. Only an exact open Codex child with a newer provider-emitted rollback count may mark that managed history generation canonical; ordinary continuation/compaction and released heuristic markers remain raw. |
...
| S11 | Pane-bounded local drag selection and automatic copy | Supported in `railmux ssh` and native Windows | Visible physical rows only; no autoscroll, soft-wrap join, or cross-pane selection. |
| S12 | URL/path hover and clean-click recognition | Conditional | Native Windows and `railmux ssh`; hover needs mouse-motion reporting and recognition fails closed outside an exact agent route. |
| S13 | Open HTTP(S) URL in the local browser | Supported on named local platforms | The remote never receives the browser action. Native Windows uses the registered URL protocol handler, never Explorer's filesystem parser. |
| S14 | Read-only path validation and managed Vim/open-directory action | Conditional | POSIX and Windows drive/UNC syntax without unquoted spaces; pane identity/path is revalidated before action. Split/swap work is serialized outside the PTY/display loop; a fresh MSYS2 shell must prove its requested cwd, and a live shell in another directory is never changed implicitly. |
...
| S17 | Bracketed paste and terminal focus-event projection | Supported | The SSH transport owns local bracketed-paste mode throughout raw interaction instead of depending on a sampled remote frame; projected focus remains allowlisted protocol state and all modes are restored on cooked prompts/exits. A bracketed paste remains opaque across local/wire read boundaries, including bounded slow marker fragmentation and embedded Railmux escape, Page, focus-like, and mouse-shaped bytes. |
| S18 | Termux compact navigation, soft-keyboard projection, touch recovery, and prompt cursor | Field-validated | Android-specific behavior is entered only from Termux environment evidence. Compact page clicks are controller-serialized and commit only after the selected agent regains full-window zoom. |
