...
| POSIX/WSL direct live scrolling | `mouse_manager.py`, `scroll_manager.py`, `tmux_ctl.py` | `test_scroll_manager.py`, focused tmux integration | Architecture: display transports and global bindings |
...
| Managed history/rewind (SSH and native Windows) | `fast_display_history.py`, `fast_display_client.py`, `fast_display_server.py`, `windows_attach_relay.LocalPtyClient`, `ui/app.App._sync_codex_rewind_scrollback` | `test_fast_display_history.py`, `test_fast_display_terminal.py`, `test_fast_display_server.py`, `test_windows_attach_relay.py`, transcript/lineage tests | Architecture: transport-managed history overlay and generation authority |
