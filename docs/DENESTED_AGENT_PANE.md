...
The full-window SSH transport is now shipped behavior, not an open part of the
swap experiment. Its terminal model, input ownership, history generations,
compatibility handshake, helper leases, and reconnect invariants live only in
`ARCHITECTURE.md`; feature acceptance and real-platform evidence live in
`SUPPORT_MATRIX.md`.

This experiment record remains authoritative only for the comparison between
`swap` and `nested`, its reproducible benchmark method, the exact transaction
and fallback observations above, and the explicit tmux 2.7/2.8 limitations.
Git history preserves the former implementation narrative.
