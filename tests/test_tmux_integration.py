...
def test_real_tmux_status_pane_range_selects_and_keeps_zoom(
    ...
    try:
        ...
        # Exercise the compact page action used by the product UI. A raw-mode
        # reader in the controller records its private sequence; the click
        # must not select/unzoom a pane before the controller performs the
        # complete park/zoom/resume transaction.
        ready_path = tmp_path / "status-action-ready"
        key_path = tmp_path / "status-action-key"
        reader = (
            "import os,termios,pathlib,time;"
            ...
            f"pathlib.Path({str(key_path)!r}).write_bytes(os.read(0,32));"
            "time.sleep(10)"
        )
        subprocess.run(
            [
                "tmux", "respawn-pane", "-k", "-t", owner_pane,
                shlex.join([sys.executable, "-c", reader]),
            ],
            check=True,
        )
        assert _wait_until(ready_path.exists)
        ...
        action_painted = bytearray()
        action_deadline = time.monotonic() + 3.0
        while (time.monotonic() < action_deadline
               and b"[1]" not in action_painted):
            readable, _, _ = select.select([master_fd], [], [], 0.1)
            if not readable:
                continue
            try:
                action_painted.extend(os.read(master_fd, 65536))
            except BlockingIOError:
                pass
        assert b"[1]" in action_painted
        # Keep this a second ordinary click rather than tmux coalescing the two
        # synthetic presses into DoubleClick1Status (a different key binding).
        time.sleep(0.6)
        os.write(master_fd, click)
        assert _wait_until(key_path.exists)
        assert key_path.read_bytes() == (
            tmux_ctl.RAILMUX_COMPACT_PAGE_KEYS[
                tmux_ctl.STATUS_ACTION_PAGE_PRIMARY
            ][1].encode()
        )
        assert tmux_ctl.active_pane_id(owner_pane) == other_pane
        assert subprocess.check_output(
            ["tmux", "display-message", "-p", "-t", other_pane,
             "#{window_zoomed_flag}"],
            text=True,
...
def test_real_tmux_swap_recovery_direct_kill_and_fallback(isolated_tmux):
    ...
    fallback_pane = fallback_workspace.primary.pane_id
    assert fallback_pane is not None
    assert tmux_ctl.pane_alive(fallback_pane)
    # One client is the independent attachment created above; the second must
    # be the nested Railmux display on this exact isolated socket. Before the
    # dedicated-socket fix, respawn-pane appeared successful but its plain
    # `tmux attach-session` reached the default server, printed "no sessions",
    # and immediately destroyed fallback_pane.
    assert _wait_until(
        lambda: tmux_ctl.session_attached_count("agent-b") == 2)
